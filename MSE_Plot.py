...
from Math_Tools import ANOVA
from Plot_Tools import assign_colors, raw_data_grid, numerical_data_grid, axes_create, colors_hex_list, axes_setup, set_xlabeling, set_xlabeling_rotation
...
def MSEPlotter_Assessor_General(
    ...
    if plot_data.error == 1:  # has error
        ...
        if plot_data.ANOVA_F is None or plot_data.ANOVA_p is None or plot_data.ANOVA_MSE is None or plot_data.F_significances is None:
            # Calculation of all p- and MSE values
            ANOVA_F, ANOVA_p, ANOVA_MSE, F_significances = ANOVA(
                s_data, plot_data)
            plot_data.ANOVA_F = ANOVA_F
            plot_data.ANOVA_p = ANOVA_p
            plot_data.ANOVA_MSE = ANOVA_MSE
            plot_data.F_significances = F_significances
            ...
            resultList = numerical_data_grid(
                s_data, plot_data, ANOVA_F, ANOVA_p, ANOVA_MSE, F_significances)
        else:
            ANOVA_F = plot_data.ANOVA_F
            ANOVA_p = plot_data.ANOVA_p
            ANOVA_MSE = plot_data.ANOVA_MSE
            F_significances = plot_data.F_significances
        ...
        for ass_ind in range(len(plot_data.activeAssessorsList)):
            ...
            for att_ind in range(len(plot_data.activeAttributesList)):
                ...
                # lines[0] = ((x0, y0), (x1, y1))
                lines.append(
                    ((position, 0), (position, ANOVA_MSE[ass_ind][att_ind])))
                ...
                pointAndLabelList.append(
                    [position, ANOVA_MSE[ass_ind][att_ind], label])

            x_positions.append((pos_max - pos_min) / 2.0 + pos_min)

        line_collection = LineCollection(
            lines, linewidths=1.2, colors=line_colors)
        ...
        if plot_data.view_legend:
            ...
            for att in activeAttributesList:
                ...
                if c_index == len(colors):
                    c_index = 0
            fig.legend(plotList, legend_list, loc='upper right')
...
def MSEPlotter_Assessor_Specific(
    s_data, plot_data, num_subplot=[
        1, 1, 1], **kwargs):
    """
    ...
    if plot_data.error == 1:  # has error
        ...
        line_collection = LineCollection(
            lines, linewidths=linewidths, colors=colors)
        # line_collection.set_linewidth(1.2)
...
def MSEPlotter_Attribute_General(
    s_data, plot_data, num_subplot=[
        1, 1, 1], **kwargs):
    """
    This function generates general F plots, plotting F values from the
    ...
    if plot_data.error == 1:  # has error
        ...
        for att_ind in range(len(plot_data.activeAttributesList)):
            ...
            for ass_ind in range(len(plot_data.activeAssessorsList)):
                ...
                line_colors.append(
                    colors[(plot_data.activeAssessorsList[ass_ind], "rep")][0])
                #c_index += 1
                #if c_index == len(colors): c_index = 0

                # Creating pointAndLabelList
                label = '(' + plot_data.activeAttributesList[att_ind] + \
...
def MSEPlotter_Attribute_Specific(
    s_data, plot_data, num_subplot=[
        1, 1, 1], **kwargs):
    """
    This function generates general F plots, plotting F values from the
