...
# ---Diverse methods:---#FFFFFF#FF8040------------------------------------
    ...
    def __init__(self, parent, name):
        ...
        self.encoding2 = sys.stdin.encoding
        ...
        try:
            ...
            self.enc, self.dec = codecs.lookup(self.encoding3)[
                :2]  # most likely using 'ascii'
    ...
    def show_msg(self, msg, _title):
        dlg = wx.MessageDialog(None, msg, _title, wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def split_path(self, abs_path):
        ...
        if os.name == 'nt':  # windows (same on a 'ce' system?)
            ...
            match = re.search(r"(.*)/(.*)\.(.*)", abs_path)
        return match.groups()
        # three items in a tuple: ('directory-path', 'filename',
        # 'file-extension')
        ...
        for rows in range(0, numRows):
            for cols in range(diff, numCols):
                self.Matrix[rows][cols -
                                  diff] = float(self.ListCollection[rows][cols])
...
# ---Error checking methods:---#FFFFFF#FF8040-----------------------------
    ...
    def missing_values_analysis(
        ...
        for i in range(len(datasheet)):
            ...
            if complete_row_missing:
                row_missing.append([i,
                                    datasheet[i][ass_index],
                                    datasheet[i][samp_index],
                                    datasheet[i][rep_index]])
    ...
    def unbalanced_data_analysis(
        ...
        for row in datasheet:
            ...
            samp = row[samp_index]
            rep = row[rep_index]
            if ass not in assessors:
                assessors.append(ass)
        ...
        for row in datasheet:
            ...
            samp = row[samp_index]
            rep = row[rep_index]
            if samp not in existing_samples[ass]:
                existing_samples[ass].append(samp)
        ...
        # dictionary with key: (assessor, sample) for the missing replicate
        missing_replicates = {}

        # find assessors with missing samples and (assessor, sample) couples
        # with missing replicate:
        for ass in assessors:
            if not len(existing_samples[ass]) == len(samples):
                if ass not in missing_assessors:
                    missing_assessors.append(ass)
                # get missing samples:
                for samp in samples:
                    if samp not in existing_samples[ass] and samp not in missing_samples:
                        missing_samples.append(samp)
            for samp in samples:
                if not len(existing_replicates[(ass, samp)]) == len(
                        replicates):
                    for rep in replicates:
                        if rep != existing_replicates[(ass, samp)]:
                            missing_replicates[(ass, samp)] = rep

        # if missing replicate: corresponding assessor or sample will be
        # removed
    ...
    def balanceData_remove_samples(
            self,
            datasheet,
            missing_samples,
            samp_index=1):
    ...
    def is_standard_file(self, firstRow):
        ...
        # only numbers? iterates firstRow[0] to firstRow[-2], because the last
        # (firstRow[-1]) will contain "\n"
        ...
        if all_are_numbers:
            self.error = "Unusal standard file or Not standard file! All " + \
                str(len(firstRow)) + " header values are numbers, check file:"
    ...
    def find_equal_columns(self, matrix, attributes):
        ...
        for attr in attributes:  # iterating attribute list
            ...
            if len(list_of_equals) > 0:
                j = 0
                # value contains the index position of the Attribute equal to
                # Attribute in attributes[i]
                for value in list_of_equals:
                    if self.equal_columns(
                        ...
                        if self.drop:
                            self.del_column(
                                value + 3)  # del from self.ListCollection and self.SparseMatrixSparseMatrix
                            self.del_from_sparseMatrix(value)
                            adjust = False
                            # adjusting list of equals, because matrix =
                            # self.ListCollection
...
# return -1
        ...
        for i in range(0, max):
            ...
            if self.isEmpty(row[i]):
                ...
                # self.showErrorDialog_YES_NO(self.error)
                    ...
                    print("Deleting column! Empty value at row-number:" +
                          str(rowNum + 1) + " column-number:" + str(i + 1))
                    # position [i-3] points to correct position in SparseMatrix
                    self.del_column(i)
                ...
                if i >= 3:
                    ...
                    if not self.isFloat(row[i]):
                        ...
                        # self.showErrorDialog_YES_NO(self.error)
                        if True:  # self.drop:
                            print("Deleting column! Not digit score-value at row-number:" +
                                  str(rowNum + 1) + " column-number:" + str(i + 1))
                            # position [i-3] points to correct position in
                            # SparseMatrix
                            self.del_column(i)
                            self.del_from_sparseMatrix(i - 3)
                            del self.AttributeList[i - 3]
            ...
            for samp in self.SampleList:
                for rep in self.ReplicateList:
                    ...
                    try:
                        ...
                        err_msg += "Missing key: (" + ass + \
                            ", " + samp + ", " + rep + ")"
                        self.showErrorDialog(err_msg)
                        return False
                    except BaseException:
                        err_msg = "Error setting row data. For key: (" + \
                            ass + ", " + samp + ", " + rep + ")"
                        self.showErrorDialog(err_msg)
                        return False
        return True
    ...
    def load_data(self, datasheet, firstRow, encode_text=False):
        ...
        missing_values_positions, rows_missing = self.missing_values_analysis(
            datasheet)
        # print(rows_missing)
        ...
        missing_ass, missing_samp, missing_rep = self.unbalanced_data_analysis(
            datasheet, ass_index, samp_index, rep_index)
        for row_missing in rows_missing:
            if row_missing[1] not in missing_ass:
                missing_ass.append(row_missing[1])
            if row_missing[2] not in missing_samp:
                missing_samp.append(row_missing[2])
        ...
        if len(missing_ass) > 0:  # unbalanced data
            ...
            self.unbalanced_data = Unbalanced_Data(
                self.parent, missing_ass, missing_samp, str_ass, str_samp)
            if self.unbalanced_data.ShowModal() == 1:
                if self.unbalanced_data.radio1_selection:
                    datasheet = self.balanceData_remove_assessors(
                        datasheet, missing_ass, ass_index)
                    ...
                    datasheet = self.balanceData_remove_samples(
                        datasheet, missing_samp, samp_index)
                    info_str = "Unbalanced data set:\n"
                    info_str += "=================\n\n"
        ...
        for row_ind in range(len(datasheet)):
            ...
            if len(row) > 3:
                if encode_text:
                    if isinstance(row[ass_index], int):
                        newAssessor = assessorExpression + \
                            ' ' + str(row[ass_index])
                    elif isinstance(row[ass_index], float):
                        newAssessor = assessorExpression + \
                            ' ' + str(row[ass_index])
                    else:
                        newAssessor = row[ass_index]

                    if isinstance(row[samp_index], int):
                        newSample = sampleExpression + \
                            ' ' + str(row[samp_index])
                    elif isinstance(row[samp_index], float):
                        newSample = sampleExpression + \
                            ' ' + str(int(row[samp_index]))
                    else:
                        newSample = row[samp_index]

                    if isinstance(row[rep_index], int):
                        newReplicate = replicateExpression + \
                            ' ' + str(row[rep_index])
                    elif isinstance(row[rep_index], float):
                        newReplicate = replicateExpression + \
                            ' ' + str(int(row[rep_index]))
                    else:
                    ...
                    if self.s_data.SparseMatrix.__contains__(
                            (newAssessor, newSample, newReplicate)):
                        self.duplicate_keys.append(
                            (newAssessor, newSample, newReplicate))
                    # pdb.set_trace()
                    self.s_data.SparseMatrix[(
                        newAssessor, newSample, newReplicate)] = floatList
            ...
            if len(self.duplicate_keys) > 0:
                ...
                for k in self.duplicate_keys:
                    str_list += k[0] + ", " + k[1] + ", " + k[2] + "\n"

                self.showErrorDialog(
                    "Cannot proceed loading. The following assessor-sample-replicate\n" +
...
class PlainText(DataFile):
    def __init__(self, parent, name, summaryFrame, delimiter):
        ...
        if self.openTextFile(self.name):
            ...
            try:
                self.text = self.get_real_UsedRange_text(
                    self.text, delimiter)  # self.text is an Array
...
class Excel(DataFile):
    def __init__(self, parent, name, summaryFrame):
        ...
        try:
            ...
            dlg = wx.MessageDialog(
                parent,
                'Cannot open file. Check that the data has correct structure and that Microsoft Excel is installed.',
                'Information',
                wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
...
class Unbalanced_Data(wx.Dialog):
    def __init__(
        ...
        self.radio2 = wx.RadioButton(
            self, -1, "Remove Samples: ", size=(400, -1))
        box_balance.Add(self.box_title, 0, wx.ALIGN_LEFT |
                        wx.ALIGN_TOP | wx.ALL, 2)
        box_balance.Add(self.radio1, 1, wx.ALIGN_LEFT |
                        wx.ALIGN_TOP | wx.ALL, 2)
        box_balance.Add(self.radio2, 1, wx.ALIGN_LEFT |
                        wx.ALIGN_TOP | wx.ALL, 2)
