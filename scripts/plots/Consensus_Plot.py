...
def PCA_plotter(
    ...
    legend = 0
    if itemID[0] == "PCA Explained Variance":
        aspect = 'auto'
    if itemID[0] == "Spiderweb Plot":
        legend = plot_data.view_legend
    ...
    if itemID[0] == u'PCA Scores':
     ...
     # Get the first and second column from the scores matrix
        ...
        # maps func(x)=round(x*10)/10.0 for all elements in list
        map(lambda x: round(x * 10) / 10.0, lims)
    ...
    # ************************************
        ...
        ax.scatter(xCorrLoadings, yCorrLoadings, s=10, c='k', marker='o')
    ...
    # ************************************
        ...
        _line = ['']
        _line.extend(activeSamplesList)
        resultList.append(_line)
        _line = ["PC" + str(pc_x + 1)]
        _line.extend(scaled_scores_x_list)
        resultList.append(_line)
        _line = ["PC" + str(pc_y + 1)]
        _line.extend(scaled_scores_y_list)
        resultList.append(_line)
        ...
        _line = ['']
        _line.extend(activeAttributesList)
        resultList.append(_line)
        _line = ["PC" + str(pc_x + 1)]
        _line.extend(scaled_loadings_x_list)
        resultList.append(_line)
        _line = ["PC" + str(pc_y + 1)]
        _line.extend(scaled_loadings_y_list)
        resultList.append(_line)
...
def STATIS_PCA_Plotter(
    ...
    if itemID[0] == "PCA Explained Variance":
        aspect = 'auto'
    if itemID[0] == "Spiderweb Plot":
        legend_on = plot_data.view_legend
