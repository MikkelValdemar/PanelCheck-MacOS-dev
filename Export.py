...
class Export_Images_Dialog(wx.Dialog):
    def __init__(
        ...
        self._text_plots = wx.StaticText(
            id=wx.NewId(),
            label=u'Plots:',
            name=u'_text_Ass',
            parent=self._panel,
        ...
        self._checkList_Ass = wx.CheckListBox(
            choices=self.AssessorList,
            id=_c_ass_id,
        ...
        self._button_Ass_EnableAll = wx.Button(
            id=_butt_ass_en_id,
            label=u'Enable all',
        ...
        self._button_Ass_DisableAll = wx.Button(
            id=_butt_ass_di_id,
            label=u'Disable all',
        ...
        self._button_Att_EnableAll = wx.Button(
            id=_butt_att_en_id,
            label=u'Enable all',
        ...
        self._button_Att_DisableAll = wx.Button(
            id=_butt_att_di_id,
            label=u'Disable all',
        ...
        self._button_Samp_EnableAll = wx.Button(
            id=_butt_samp_en_id,
            label=u'Enable all',
        ...
        self._button_Samp_DisableAll = wx.Button(
            id=_butt_samp_di_id,
            label=u'Disable all',
        ...
        if saving_ppt_file:
            ...
            self.input_browse_images = wx.TextCtrl(
                id=browse_images, parent=self._panel, pos=wx.Point(
                    4, 350), size=wx.Size(
                    210, 28))
            self.input_browse_images.SetValue(export_path)
            self.button_browse_im = wx.Button(
                id=browse_images, label=u'...', parent=self._panel, pos=wx.Point(
                    218, 350), size=wx.Size(
                    32, 28))
            self.button_browse_im.Bind(
                wx.EVT_BUTTON,
                self.onButtonBrowseIm,
                id=browse_images)
            self.button_browse_im.SetToolTipString(
                u'Select image folder for current PowerPoint file')
        else:
            self.radio1 = wx.RadioButton(
                self._panel, -1, ".PNG", style=wx.RB_GROUP, pos=wx.Point(4, 320), size=(-1, -1))
            self.radio2 = wx.RadioButton(
    ...
    def OnButtonOK(self, event):
        activeAssessors_List = self.get_ActivesFromCheckBoxList(
            self.AssessorList, self._checkList_Ass)
        activeAttributes_List = self.get_ActivesFromCheckBoxList(
            self.AttributeList, self._checkList_Att)
        activeSamples_List = self.get_ActivesFromCheckBoxList(
            self.SampleList, self._checkList_Samp)
        active_plots = self.get_ActivesFromCheckBoxList(
            self.plots, self._checkList_plots)
        ...
        if len(active_plots) < 1 and not (
                self.saving_ppt_file and selection == 1):
            dlg = wx.MessageDialog(
                None,
                'No plots selected. Select from the Plots list and click again to save plots as image files.',
                'Error Message',
        ...
        if self.saving_ppt_file:
            ...
            try:
                ...
                dlg = wx.MessageDialog(
                    None,
                    'Cannot process with current DPI value.\nPlease change DPI value.',
                    'Error Message',
                    wx.OK | wx.ICON_INFORMATION)
                dlg.ShowModal()
                dlg.Destroy()
            ...
            if dpi > 4000:
                ...
                dlg = wx.MessageDialog(
                    None,
                    'The DPI value cannot be less than 1.\nPlease change DPI value.',
                    'Error Message',
                    wx.OK | wx.ICON_INFORMATION)
                dlg.ShowModal()
                dlg.Destroy()
    ...
    def update_selection(self, changes):
        ...
        if changes.__contains__("att"):
            self.check_CheckBoxList(
                self.AttributeList, self._checkList_Att, False)
            for ind in changes["att"]:
                self._checkList_Att.Check(ind, True)

        if changes.__contains__("samp"):
            self.check_CheckBoxList(
                self.SampleList, self._checkList_Samp, False)
