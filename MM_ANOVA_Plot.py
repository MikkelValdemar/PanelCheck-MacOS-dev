...
from Plot_Tools import check_columns, show_info_msg
...
def load_mm_anova_data(s_data, plot_data, one_rep=False, abspath=None):
    ...
    if 'sensmixed_data' not in plot_data.__dict__.keys() or plot_data.sensmixed_data is None:
        ...
        matrix_selected_scores, out_cols = check_columns(
            matrix_selected_scores)
...
def get_grid_data(s_data, plot_data, f_mat, p_mat, lsd_mat, plot_type):
    ...
    if plot_data.raw_data == []:
        ...
        m_data = s_data.GetActiveData(
            active_assessors=activeAssessorsList,
            active_samples=activeSamplesList,
        ...
        if plot_type == "2way1rep":
            ...
            for ind in range(len(f_mat)):
                ...
                _index += 1
                numeric_data[_index].extend(str_row(f_mat[ind]))
                numeric_data.append(["p-values"])
                _index += 1
                numeric_data[_index].extend(
                    str_row(p_mat[ind], fmt="%.3f"))  # p_matr2
                numeric_data.append(newline)
                _index += 1

        numeric_data.append(["LSD"])
        _index += 1
        numeric_data[_index].extend(str_row(lsd_mat[0]))
        numeric_data.append(["Bonferroni LSD"])
        _index += 1
...
def MixModel_ANOVA_Plotter_2way1rep(
    ...
    if len(s_data.ReplicateList) > 1:  # no active assessors
        dlg = wx.MessageDialog(
            None,
            'This data set contains more than one Replicate. Use 2- or 3-way ANOVA.',
            'Error Message',
            wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    if len(activeSamplesList) < 2:  # no active samples
        dlg = wx.MessageDialog(
            None,
            'A minimum of two Samples must be active.',
            'Error Message',
            wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    rawDataList, resultList = get_grid_data(
        s_data, plot_data, f_list, p_list, lsd_list, "2way1rep")
...
def MixModel_ANOVA_LSD_Plotter_2way1rep(
    ...
    for samp in activeSamplesList:
        ...
        for att in activeAttributesList:
            att_column = s_data.GetAttributeData(
                activeAssessorsList, att, samp)
            if stacking == 0:
...
def MixModel_ANOVA_Plotter_2way(
    ...
    if len(s_data.ReplicateList) < 2:  # no active assessors
        dlg = wx.MessageDialog(
            None,
            'Cannot process: There must be a minimum of two Replicates per Sample in the data set',
            'Error Message',
            wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
...
# selected [1, ..., n]
def set_points_in_range(ax, ydata, selected, point_list):
...
def MixModel_ANOVA_OverviewPlotter(
        s_data,
        plot_data,
        *args,
        abspath=None,
...
def MixModel_ANOVA_LSD_OverviewPlotter(
        s_data,
        plot_data,
        abspath=None,
        *args,
