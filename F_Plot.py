...
from Plot_Tools import OverviewPlotter, set_xlabeling, num2str, axes_create, assign_colors, check_point, axes_setup, set_xlabeling_rotation, raw_data_grid, numerical_data_grid
    ...
    if itemID[1] not in collectedActiveItemsList:
        ...
        for ass_ind in range(len(ANOVA_F)):
            for att_ind in range(len(ANOVA_F[0])):
                ...
                if isinstance(
                        F_value, (int, float)) and isinstance(
                        p_value, (int, float)):
                    # find max values
                    if F_value > max_F:
                        max_F = F_value
        ...
        if itemID[0] == "F-values":
            lines2.append(
                ((0, F_significances[0]), (max_x_scale, F_significances[0])))
            line_colors2.append("#ff0000")
            lines2.append(
                ((0, F_significances[1]), (max_x_scale, F_significances[1])))
        ...
        axes_setup(
            ax, '', _ylabel, myTitle, [
                min_x_scale, max_x_scale, 0, ymax])
...
def FPlotter_Assessor_Specific(
    ...
    if itemID[1] not in collectedActiveItemsList:
        ...
        line_collection = LineCollection(
            lines, colors=line_colors, linewidths=linewidths)
        ax.add_collection(line_collection)
...
def FPlotter_Attribute_General(
    s_data, plot_data, num_subplot=[
        1, 1, 1], **kwargs):
    """
    This function generates general F plots, plotting F values from the
...
def FPlotter_Attribute_Specific(
    s_data, plot_data, num_subplot=[
        1, 1, 1], **kwargs):
    """
    This function generates general F plots, plotting F values from the
