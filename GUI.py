...
class Main_Frame(wx.Frame):
    ...
    def _init_coll_menuFileImport_Items(self, parent):
        ...
        parent.Append(
            id=cd_id,
            kind=wx.ITEM_NORMAL,
            item=u'Plain Text (Comma delimited)')
        parent.Append(
            id=sd_id,
            kind=wx.ITEM_NORMAL,
            item=u'Plain Text (Semicolon delimited)')
    ...
    def _init_coll_menuView_Items(self, parent):
        ...
        self.menuOptionsGrid = self.menuViewPlot.Append(
            id=wxID_MAIN_FRAMEMENUVIEWGRID, kind=wx.ITEM_CHECK, item=u'View Grid')
        self.menuOptionsLegend = self.menuViewPlot.Append(
            id=wxID_MAIN_FRAMEMENUVIEWLEGEND, kind=wx.ITEM_CHECK, item=u'View Legend')

        self.menuViewSelect = wx.Menu(title='')
        selectAll_id = wx.NewId()
        self.menuOptionsSelectAll = self.menuViewSelect.Append(
            id=selectAll_id, kind=wx.ITEM_CHECK, item=u'Equal for all Plots')
    ...
    def _init_ctrls(self, prnt):
        ...
        self.main_notebook = wx.Notebook(
            id=wx.NewId(), name=u'main_notebook', parent=self, pos=wx.Point(
                0, 0), size=wx.Size(
                852, 465), style=wx.NB_FIXEDWIDTH | wx.TAB_TRAVERSAL | wx.STRETCH_NOT)

        self.uni_notebook = wx.Notebook(
            id=wx.NewId(), name=u'uni_notebook', parent=self.main_notebook, pos=wx.Point(
                0, 0), size=wx.Size(
                744, 436), style=0)

        self.multi_notebook = wx.Notebook(
            id=wx.NewId(), name=u'multi_notebook', parent=self.main_notebook, pos=wx.Point(
                0, 0), size=wx.Size(
                744, 436), style=0)

        self.avgr_notebook = wx.Notebook(
            id=wx.NewId(), name=u'avgr_notebook', parent=self.main_notebook, pos=wx.Point(
                0, 0), size=wx.Size(
                744, 436), style=0)

        self.overall_notebook = wx.Notebook(
            id=wx.NewId(),
            name=u'overall_notebook',
        ...
        self.choice_label = wx.StaticText(
            id=wx.NewId(),
        ...
        self.line_panel = TabPanel(
            parent=self.uni_notebook,
            func=self.create_plot,
            choice=self.choice)

        self.mean_std_panel = TabPanel(
            parent=self.uni_notebook,
            func=self.create_plot,
            choice=self.choice)

        self.corr_panel = TabPanel(
            parent=self.uni_notebook,
            func=self.create_plot,
            choice=self.choice)

        self.profile_panel = RadioTabPanel(
            parent=self.uni_notebook,
            func_list=[
                self.create_plot,
            ...
            radiobox_choices=[
                ...
                "Sample Replicates"],
            radio_label="Show:")
        self.profile_radioBoxModel = wx.RadioBox(
            choices=[
                "Increasing intensity",
                "Original sample order"],
            id=-1,
            label="Sorting:",
            parent=self.profile_panel,
            size=wx.DefaultSize,
            style=wx.RA_SPECIFY_COLS)
        self.profile_panel.lower_right_sizer.AddSpacer(5)
        self.profile_panel.lower_right_sizer.Add(
            self.profile_radioBoxModel, 0, wx.STRETCH_NOT)

        self.egg_panel = TabPanel(
            parent=self.uni_notebook,
            func=self.create_plot,
            choice=self.choice)

        self.f_panel = RadioTabPanel(
            parent=self.uni_notebook,
            func_list=[
                self.create_plot,
                self.show_fPlot_tree_assessor,
                self.show_fPlot_tree_attribute],
            choice=self.choice,
            radiobox_choices=[
                "Assessors",
                "Attributes"],
            radio_label="Sorted by:")

        self.mse_panel = RadioTabPanel(
            parent=self.uni_notebook,
            func_list=[
                self.create_plot,
                self.show_msePlot_tree_assessor,
                self.show_msePlot_tree_attribute],
            choice=self.choice,
            radiobox_choices=[
                "Assessors",
                "Attributes"],
        ...
        self.pmse_panel = TabPanel(
            parent=self.uni_notebook,
            func=self.create_plot,
            choice=self.choice)

        self.tuck1_panel = RadioTabPanel(
            parent=self.multi_notebook,
            func_list=[
                self.create_plot,
            ...
            radiobox_choices=[
                "None",
                "Standardize"],
            radio_label="Variable standardization:")

        box = wx.StaticBox(self.tuck1_panel, 0, "Axis options:")
        bsizer = wx.StaticBoxSizer(box, wx.HORIZONTAL)
        self.tuck1_cb = wx.CheckBox(self.tuck1_panel, 0, label="Equal axis")
        self.tuck1_panel.lower_right_sizer.AddSpacer(5)
        bsizer.AddSpacer(5)
        bsizer.Add(self.tuck1_cb, 0, wx.FIXED_MINSIZE |
                   wx.ALIGN_CENTER_VERTICAL)
        #bsizer.Add(self.tuck1_cb, 0, wx.GROW|wx.ALIGN_CENTER_VERTICAL)
        bsizer.AddSpacer(5)
        self.tuck1_radioBoxModel = wx.RadioBox(
            choices=[
                "Sample averages",
                "Sample replicates"],
            id=-1,
            label="Model based on:",
            parent=self.tuck1_panel,
            size=wx.DefaultSize,
            style=wx.RA_SPECIFY_COLS)
        self.tuck1_panel.lower_right_sizer.Add(
            self.tuck1_radioBoxModel, 0, wx.STRETCH_NOT)
        self.tuck1_panel.lower_right_sizer.AddSpacer(5)
        self.tuck1_panel.lower_right_sizer.Add(bsizer, 0, wx.STRETCH_NOT)

        self.manh_panel = RadioTabPanel(
            parent=self.multi_notebook,
            func_list=[
                self.create_plot,
        ...
        box = wx.StaticBox(self.manh_panel, 0, "Max. PCs:")
        bsizer = wx.StaticBoxSizer(box, wx.HORIZONTAL)
        self.manh_spin_txt = wx.TextCtrl(
            self.manh_panel, 0, size=(
                50, 25), value="6", style=wx.TE_READONLY)
        self.manh_spin = wx.SpinButton(
            self.manh_panel, 0, size=(
                25, 25), style=wx.SP_VERTICAL)
        ...
        bsizer.Add(self.manh_spin, 0, wx.FIXED_MINSIZE |
                   wx.ALIGN_CENTER_VERTICAL)
        bsizer.AddSpacer(5)
        self.manh_panel.lower_right_sizer.Add(bsizer, 0, wx.STRETCH_NOT)
        self.Bind(wx.EVT_SPIN, self.onManhSpin, self.manh_spin)

        self.org_panel = TabPanel(
            parent=self.avgr_notebook,
            func=self.create_plot,
            choice=self.choice)
        box = wx.StaticBox(self.org_panel, 0, "Axis options:")
        bsizer = wx.StaticBoxSizer(box, wx.HORIZONTAL)
        self.org_cb = wx.CheckBox(self.org_panel, 0, label="Equal axis")
        ...
        self.std_panel = TabPanel(
            parent=self.avgr_notebook,
            func=self.create_plot,
            choice=self.choice)
        box = wx.StaticBox(self.std_panel, 0, "Axis options:")
        bsizer = wx.StaticBoxSizer(box, wx.HORIZONTAL)
        self.std_cb = wx.CheckBox(self.std_panel, 0, label="Equal axis")
        ...
        self.statis_panel = RadioTabPanel(
            parent=self.avgr_notebook,
            func_list=[
                self.create_plot,
            ...
            radiobox_choices=[
                "Covariance",
                "Correlation"],
            radio_label="STATIS based on:")
        box = wx.StaticBox(self.statis_panel, 0, "Axis options:")
        bsizer = wx.StaticBoxSizer(box, wx.HORIZONTAL)
        self.statis_cb = wx.CheckBox(self.statis_panel, 0, label="Equal axis")
        self.statis_panel.lower_right_sizer.AddSpacer(5)
        bsizer.AddSpacer(5)
        bsizer.Add(self.statis_cb, 0, wx.FIXED_MINSIZE |
                   wx.ALIGN_CENTER_VERTICAL)
        bsizer.AddSpacer(5)
        self.statis_panel.lower_right_sizer.Add(bsizer, 0, wx.STRETCH_NOT)

        self.mm_anova_panel1 = TabPanel(
            parent=self.overall_notebook,
            func=self.create_plot,
            choice=self.choice)

        self.mm_anova_panel2 = TabPanel(
            parent=self.overall_notebook,
            func=self.create_plot,
            choice=self.choice)

        self.mm_anova_panel3 = TabPanel(
            parent=self.overall_notebook,
            func=self.create_plot,
            choice=self.choice)
    ...
    def __init__(self, parent, filename, delimiter='\t'):
        super(Main_Frame, self).__init__(parent=parent)
    ...
    def add_recent_files(self):
        ...
        for ind in range(num_tot):
            ...
            self.menuFileRecent.Prepend(
                id=recent_id, kind=wx.ITEM_NORMAL, item=filename)
    ...
    def update_tabs(self, setting_limits=True):
        ...
        # set lists
        self.line_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.mean_std_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.corr_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.profile_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.egg_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.f_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.mse_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.pmse_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)

        self.tuck1_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.manh_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)

        self.org_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.std_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.statis_panel.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)

        self.mm_anova_panel1.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.mm_anova_panel2.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
        self.mm_anova_panel3.set_lists(
            self.s_data.AssessorList,
            self.s_data.AttributeList,
            self.s_data.SampleList)
    ...
    def show_perf_ind_tree(self):
        ...
        root = tree.AppendItem(
            root_sd, u'Standardized performance indices (only AGR and REP)')
        tree.SetItemData(root, None)
        child = tree.AppendItem(root, u'Indices table')
    ...
    def OnGeneralFileOpen(self, fileName, delimiter):
        ...
        if (fileName[-4:] == ".xls" or fileName[-5:] == ".xlsx" or fileName[-4:] == ".xlw" or fileName[
                                                                                              -4:] == ".wk1" or fileName[
                                                                                                                -4:] == ".wk3" or fileName[
                                                                                                                                  -4:] == ".wk4"):
