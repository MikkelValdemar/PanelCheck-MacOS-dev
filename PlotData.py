...
class PlotData:
    ...
    def __init__(self, active_ass=[], active_att=[], active_samp=[],
        ...
        # numerical data of the calculations (for viewing in grid frame)
        self.numeric_data = []
    ...
    def actives_changed(self, active_ass=[], active_att=[], active_samp=[]):
        ...
        """
        if not self.activeAssessorsList == active_ass:
            return True  # not equal, actives changed
        if not self.activeAttributesList == active_att:
            return True  # not equal, actives changed
        if not self.activeSamplesList == active_samp:
            return True  # not equal, actives changed
...
class MM_ANOVA_PlotData(PlotData):
    ...
    def copy_data(self, obj):  # copy from obj
        ...
        self.accepted_active_attributes = deepcopy(
            obj.accepted_active_attributes)
...
class ANOVA_PlotData(PlotData):
    ...
    def copy_data(self, obj):  # copy from obj
        ...
        self.F_significances = deepcopy(obj.F_significances)
...
class PCA_PlotData(PlotData):
    ...
    def copy_data(self, obj):  # copy from obj
        ...
        self.raw_data = deepcopy(obj.raw_data)
        self.raw_data_mv_pos = deepcopy(obj.raw_data_mv_pos)
        self.numeric_data_tucker1matrix = deepcopy(
            obj.numeric_data_tucker1matrix)
        self.p_matr = deepcopy(obj.p_matr)
...
class CollectionCalcPlotData(PlotData):
    def __init__(
        ...
        # for general purpose use (i.e. as Dict of variables)
        self.collection_calc_data = {}
