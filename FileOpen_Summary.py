...
class Summary(wx.Dialog):
    def _init_ctrls(self, prnt):
        ...
        self.ch_ass = wx.Choice(
            self.panel1, -1, choices=grid_col_names, size=(90, -1))
        self.ch_samp = wx.Choice(
            self.panel1, -1, choices=grid_col_names, size=(90, -1))
        self.ch_rep = wx.Choice(
            self.panel1, -1, choices=grid_col_names, size=(90, -1))
        ...
        self.list_columns = wx.CheckListBox(
            parent=self.panel1, id=-1, size=(130, 100))
        ...
        self.buttonCancel1 = wx.Button(
            id=cancel1, label=u'Cancel', parent=self.panel1)
        self.buttonCancel1.Bind(
            wx.EVT_BUTTON,
            self.OnCancel1Button,
            id=cancel1)
        ...
        box_buttons1.Add(self.buttonCancel1, 0, wx.ALIGN_BOTTOM | wx.ALL, 2)
        box_buttons1.Add(self.buttonOK1, 0,
                         wx.ALIGN_BOTTOM | wx.ALL, 2)

        self.sizer_select.Add(box_buttons1, 0, wx.ALL, 0)
        ...
        self.textLabelLims = wx.StaticText(
            id=wx.NewId(),
            label=u'Data set min/max :',
            name=u'textLabelLims',
            parent=self.panel3)
        self.textSummary3 = wx.TextCtrl(
            id=wx.NewId(),
            name=u'textSummary',
            parent=self.panel3,
        ...
        box_inner3a.Add(self.buttonOK3, 0, wx.ALIGN_BOTTOM | wx.ALL, 2)

        self.sizer_summary.Add(
            self.textSummary3, 1, wx.LEFT | wx.TOP | wx.GROW)
    ...
    def OnButtonOK3Button(self, event):
        if self.checkValues():
            ...
            dlg = wx.MessageDialog(
                self,
                'Values not accepted. Change one or both of the minimum and maximum scale values.',
                'Error Message',
    ...
    def checkValues(self):
        try:
            # just a big number, chosen randomly by HR (06.08.2005)
            limit = 65536
            if float(self.textMinInput.GetValue()) > limit * (-1) and float(
                    self.textMinInput.GetValue()) < float(self.textMaxInput.GetValue()):
    ...
    def checkSelections(self):
        ...
        if self.ass_index == self.samp_index or self.ass_index == self.rep_index or self.rep_index == self.samp_index:
            dlg = wx.MessageDialog(
                self,
                'Two or more of the column-selectors have been assigned to same column. Please change column selections.',
                'Error Message',
                wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            ...
            dlg = wx.MessageDialog(
                self,
                'You have assigned to last column, there will be no score values. Please change column selections or check file.',
                'Error Message',
                wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def set_limits(self, limits):
        self.limits = limits
        # 4% of length lower to upper limits
        self.epsilon = -1 * \
            (-1 * self.limits[3] - (-1 * self.limits[2])) * 0.04
