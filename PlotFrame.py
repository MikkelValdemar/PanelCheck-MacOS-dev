...
from PlotPanel import PlotPanel, PointLabel
...
class PlotFrame(wx.Frame):
    ...
    def __init__(self, parent, fig_title, s_data, plot_data, mother, abspath):
        ...
        # self.plot_panel.canvas.Bind(wx.EVT_LEFT_UP,
        # self.on_panel_left_release) # release_event

        # print self.plot_panel.canvas.bitmap
        self.plot_panel.subplot = self.plot_data.ax
        self.plot_panel.canvas.mpl_connect(
            'motion_notify_event', self.mouse_move)
        self.plot_panel.canvas.mpl_connect(
            'button_press_event', self.mouse_click)
        self.plot_panel.canvas.mpl_connect(
            'button_release_event', self.mouse_release)
        # self.plot_panel.canvas.mpl_connect('key_press_event',
        # self.on_key_event) # for CTRL+C, copy
        ...
        # 0.65% of x-edge length
        self.epsilonX = -1 * (-1 * xLims[1] - (-1 * xLims[0])) * 0.0065
        # 0.95% of y-edge length
        self.epsilonY = -1 * (-1 * yLims[1] - (-1 * yLims[0])) * 0.0095
        ...
        if self.plot_type == "tucker1":  # tucker1 has special configuration for plot.
            # for Tucker-1 plotConfig[2] is used as PCA mode selection and
            # plotConfig[3] for max number of PCs
        ...
        if self.plot_type == "consensus":  # consensus has special configuration for plot.
            ...
            self.maxPCs = self.plot_data.max_PCs
        # consensus has special configuration for plot.
        if self.plot_type == "statis_consensus":
            # for Consensus plotConfig[2] is used as tab selection
        ...
        if self.labeling_type == 1:
            ...
            if self.plot_type == "mm_anova_f_2way1rep" or self.plot_type == "mm_anova_f_2way" or self.plot_type == "mm_anova_f_3way" or self.plot_type == "mean_std_att" or self.plot_type == "mean_std_ass":
                ...
                # 0.25% of x-edge length
                self.epsilonX = -1 * (-1 * xLims[1] - (-1 * xLims[0])) * 0.0025
        ...
        buttUp_id = wx.NewId()
        buttDown_id = wx.NewId()
        buttReset_id = wx.NewId()
        buttLeft_id = wx.NewId()
        buttRight_id = wx.NewId()

        buttPrevPlot_id = wx.NewId()
        ...
        self.butt_zoom_reset = wx.BitmapButton(
            ...
            size=wx.Size(
                ...
                24),
            style=wx.BU_AUTODRAW,
            id=butt_zoom_reset_id)
        self.butt_zoom_reset.SetToolTip(u'Reset Axes')

        self.butt_zoom = buttons.GenBitmapToggleButton(
            parent=self.button_panel,
            id=butt_zoom_id,
            bitmap=wx.Bitmap(
                self.ProgPathAbs +
        ...
        self.butt_pan = buttons.GenBitmapToggleButton(
            parent=self.button_panel,
            id=butt_pan_id,
            bitmap=wx.Bitmap(
                self.ProgPathAbs +
        ...
        self.button_raw = wx.Button(
            self.button_panel, butt_rawData, "Raw Data")
        self.button_res = wx.Button(
            self.button_panel,
            butt_numResult,
            "Numerical Results")
        ...
        self.buttPrevPlot = wx.Button(
            self.button_panel,
            buttPrevPlot_id,
            "<< Prev. Plot")
        self.buttNextPlot = wx.Button(
            self.button_panel, buttNextPlot_id, "Next Plot >>")
        ...
        self.box.Add(self.button_panel, 0, wx.EXPAND)
            ...
            if self.dclick_data is not None:
                if self.plot_type == "pmse":
                    ...
                    if self.plot_type == "mm_anova_f_3way" or self.plot_type == "mm_anova_f_2way":
                        plot_data.collection_calc_data["p_matr"] = deepcopy(
                            self.plot_data.p_matr)
                    plot_data.set_limits(self.plot_data.limits)
                    plot_data = plotter(self.s_data, plot_data)
    ...
    def on_zoom_reset(self, event):
        self.plot_panel.subplot.set_xlim(
            self.original_xLims[0], self.original_xLims[1])
        self.plot_panel.subplot.set_ylim(
            self.original_yLims[0], self.original_yLims[1])
        self.recalc_epsilon()
    ...
    def get_plot_method(self, plot_type):
        if plot_type == "line_samp":
            return SampleLinePlotter

        elif plot_type == "line_ass":
            return AssessorLinePlotter
    ...
    def mouse_move(self, event):
        ...
        if self.lsd_plot and self.lsd_line_selected > -1:  # case: lsd plot
            ...
            if self.labeling_type == 0:
                if self.plot_type == "manhattan_ass" or self.plot_type == "manhattan_att":
                    ax = event.inaxes
                    if ax == self.plot_panel.figure.axes[1]:
                        return
        ...
        if not self.overview_plot:
            if event.inaxes is None:
                ...
                if self.lsd_plot:
                    ...
                    self.lsd_line_selected = self.LSD_select_line(
                        self.m_x, self.m_y)
            ...
            plot_data = self.newplot(
                self.plot_type, idx[(ax.rowNum, ax.colNum)])
            plot_data.set_limits(self.plot_data.limits)
        ...
        if self.toolTipText != "":
            if self.p is not None:
                if self.p.orig_txt != self.toolTipText:
                    ...
                    self.p = PointLabel(
                        self.plot_panel, point, self.toolTipText)
    ...
    def onExport(self, event=None):
        ...
        if self.mother.image_save_path == "":
            thisdir = os.getcwd()
        else:
            thisdir = self.mother.image_save_path
    ...
    def printLabelIfCorrectPoint(self, x, y):
        ...
        for innerList in self.points_labels:
            if innerList[0] < x + \
                    self.epsilonX and innerList[0] > x - self.epsilonX:
                if innerList[1] < y + \
                        self.epsilonY and innerList[1] > y - self.epsilonY:
                    self.statusBar.SetStatusText(
                        "x=%f  y=%f" %
                        (innerList[0], innerList[1]) + "     " + innerList[2])
                    self.toolTipText = "x=%.3f  y=%.3f" % (
                        innerList[0], innerList[1]) + "\n" + innerList[2]
                    if self.plot_type == "pmse":
                        self.dclick_data = [innerList[3], innerList[4]]
                    elif self.plot_type == "corr":
                        self.dclick_data = [
                            innerList[3], innerList[4], innerList[5]]
                    elif self.plot_type == "tucker1":
                        if len(innerList) >= 4:
    ...
    def get_pos(self, value, check_list, index):
        ...
        for element in check_list:
            if value == element:
                ...
                break
            i += 1
        if j == len(check_list) - 1 and index == 1:
            return 0
        else:
            return j + index
    ...
    def get_tree_path(self, plot_type, index, index_is_pos=False):
        ...
        if plot_type == "line_samp":
            ...
            if index_is_pos:
                ...
                i = self.get_pos(
                    self.plot_data.tree_path[2],
                    self.s_data.ReplicateList,
                    index)
            tree_path.append(self.plot_data.tree_path[0])
            ...
            if index_is_pos:
                ...
                i = self.get_pos(
                    self.plot_data.tree_path[0], roller_list, index)
            tree_path.append(roller_list[i])
            # set correct plot type:
            if tree_path[0] in self.active_ass:
                self.plot_type = "mean_std_ass"
                self.plot_data.plot_type = "mean_std_ass"
            else:
                self.plot_type = "mean_std_att"
                self.plot_data.plot_type = "mean_std_att"
            ...
            if self.plot_data.tree_path[1] in self.s_data.AttributeList:
                ...
                if index_is_pos:
                    ...
                    i = self.get_pos(
                        self.plot_data.tree_path[1], self.active_ass, index)
                tree_path.append(self.plot_data.tree_path[0])
                tree_path.append(self.active_ass[i])
            ...
            if index_is_pos:
                ...
                i = self.get_pos(self.plot_data.tree_path[1], actives, index)
            tree_path.append(self.plot_data.tree_path[0])
            ...
            if index_is_pos:
                ...
                i = self.get_pos(self.plot_data.tree_path[0], actives, index)
            tree_path.append(actives[i])
            ...
            actives = self.active_ass[:]
            # actives now contain active assessors followed by active
            # attributes
            actives.extend(self.active_att[:])
            if index_is_pos:
                i = index
            else:
                i = self.get_pos(self.plot_data.tree_path[0], actives, index)
            ...
            # actives now contain 'Common Scores' and active assessors followed
            # by active attributes
            actives.extend(self.active_att)
            if index_is_pos:
                i = index
            else:
    ...
    def replot_pc(self, plot_type, pc_x, pc_y):
        if plot_type == "tucker1":
            ...
            self.plot_data = Tucker1Plotter(
                self.s_data,
                self.plot_data,
                selection=self.PCAmode,
                pc_x=pc_x,
                pc_y=pc_y,
                abspath=self.ProgPathAbs)
        elif plot_type == "consensus":
            self.plot_panel.figure.clear()
            self.plot_data = PCA_plotter(
                self.s_data,
                self.plot_data,
    ...
    def newplot(self, plot_type, index):
        ...
        if plot_type == "mm_anova_f_2way1rep" or plot_type == "mm_anova_lsd_2way1rep" or plot_type == "mm_anova_f_2way" or plot_type == "mm_anova_lsd_2way" or plot_type == "mm_anova_f_3way" or plot_type == "mm_anova_lsd_3way":
            ...
            return plotter(
                self.s_data,
                plot_data,
                selection=self.plot_data.selection)

        elif plot_type == "pmse":
            plot_data = ANOVA_PlotData(
                self.active_ass,
                self.active_att,
                self.active_samp,
                tree_path,
    ...
    def replot(self, plot_type, index):
        ...
        if plot_type == "line_samp":
            ...
            self.plot_data = RawDataAssessorPlotter(
                self.s_data, self.plot_data, abspath=self.ProgPathAbs)

        elif plot_type == "mean_std_att":
            self.plot_data = RawDataAttributePlotter(
                self.s_data, self.plot_data, abspath=self.ProgPathAbs)
            ...
            if i == 0:
                self.plot_data = FPlotter_Assessor_General(
                    self.s_data, self.plot_data)
            else:
                self.plot_data = FPlotter_Assessor_Specific(
                    self.s_data, self.plot_data)

        elif plot_type == "f_att":
            if i == 0:
                self.plot_data = FPlotter_Attribute_General(
                    self.s_data, self.plot_data)
            else:
                self.plot_data = FPlotter_Attribute_Specific(
                    self.s_data, self.plot_data)

        elif plot_type == "mse_ass":
            if i == 0:
                self.plot_data = MSEPlotter_Assessor_General(
                    self.s_data, self.plot_data)
            else:
                self.plot_data = MSEPlotter_Assessor_Specific(
                    self.s_data, self.plot_data)

        elif plot_type == "mse_att":
            if i == 0:
                self.plot_data = MSEPlotter_Attribute_General(
                    self.s_data, self.plot_data)
            else:
                self.plot_data = MSEPlotter_Attribute_Specific(
                    self.s_data, self.plot_data)

        elif plot_type == "pmse":
            self.plot_data = pmsePlotter(self.s_data, self.plot_data, self.ProgPathAbs)
            ...
            self.plot_data = Tucker1Plotter(
                ...
                self.plot_data,
                selection=self.PCAmode,
                pc_x=self.pc_x_index,
                pc_y=self.pc_y_index,
                abspath=self.ProgPathAbs)
            ...
            self.plot_data = PCA_plotter(
                ...
                selection=self.average_mode,
                pc_x=self.pc_x_index,
                pc_y=self.pc_y_index)  # 0: PC1, 1: PC2

        elif plot_type == "statis_consensus":
            if self.plot_data.tree_path[0] == u'Assessor Weights':
                self.plot_data = STATIS_AssWeight_Plotter(
                    self.s_data, self.plot_data, selection=self.statis_mode)  # 0: PC1, 1: PC2
            else:
                self.plot_data = STATIS_PCA_Plotter(
                    self.s_data,
                    self.plot_data,
            ...
            self.plot_data = MixModel_ANOVA_Plotter_2way1rep(
                self.s_data, self.plot_data)

        elif plot_type == "mm_anova_lsd_2way1rep":
            self.plot_data = MixModel_ANOVA_LSD_Plotter_2way1rep(
                self.s_data, self.plot_data)

        elif plot_type == "mm_anova_f_2way":
            self.plot_data = MixModel_ANOVA_Plotter_2way(
                self.s_data, self.plot_data)

        elif plot_type == "mm_anova_lsd_2way":
            self.plot_data = MixModel_ANOVA_LSD_Plotter_2way(
                self.s_data, self.plot_data)

        elif plot_type == "mm_anova_f_3way":
            self.plot_data = MixModel_ANOVA_Plotter_3way(
                self.s_data, self.plot_data)

        elif plot_type == "mm_anova_lsd_3way":
            self.plot_data = MixModel_ANOVA_LSD_Plotter_3way(
                self.s_data, self.plot_data)
