...
from Plot_Tools import OverviewPlotter, set_xlabeling, num2str, axes_create, assign_colors, check_point, axes_setup, set_xlabeling_rotation, raw_data_grid, colored_frame
...
def SampleLinePlotter(s_data, plot_data, num_subplot=[1, 1, 1], abspath=None, **kwargs):
    ...
    plottingMatrix = np.reshape(np.array(flatMatrixList, float),
                             (len(plottingSparseMatrix), -1))
    meanVector = np.average(plottingMatrix, 0)
    ...
    for eachAttribute in range(dimensions[1]):
        column = np.take(plottingMatrix, (eachAttribute,), 1)
        # print column
        ...
        deviation = round(np.std(column), 3)
    ...
    x_values = np.arange(1, dimensions[1] + 1)
    y_values = meanVector
...
# enableds_data.AssessorList.sort()
    ...
    # drawSettings[3][1] = 1
    maxPlotsBesideEachother = int(np.floor((100 / plot_data.limits[1]) - 1))
    print(
...
def AssessorLinePlotter(s_data, plot_data, num_subplot=[1, 1, 1], abspath=None, **kwargs):
    ...
    if itemID[1] not in activeAssessorsList:
        ...
        # Here the flatMatrixList is converted into the plottingMatrix array.
        plottingMatrix = np.reshape(
            np.array(flatMatrixList, float), (len(plottingSparseMatrix), -1))
        meanVector = np.average(plottingMatrix, 0)
        dimensions = plottingMatrix.shape
        ...
        assessorPlottingMatrix = np.reshape(
            np.array(assessorFlatMatrixList, float), (len(assessorPlottingSparseMatrix), -1))
        assessorMeanVector = np.average(assessorPlottingMatrix, 0)
        ...
        for eachAttribute in range(assessorDimensions[1]):
            column = np.take(assessorPlottingMatrix, (eachAttribute,), 1)
            # print column
            lowestValue = min(column)
            highestValue = max(column)
            deviation = round(np.std(column), 3)
...
def ReplicateLinePlotter(s_data, plot_data, abspath=None, **kwargs):
    ...
    if itemID[1] not in activeAssessorsList:
        ...
        replicatePlottingMatrix = np.reshape(
            np.array(replicateFlatMatrixList, float), (len(replicatePlottingSparseMatrix), -1))
        replicateMeanVector = np.average(replicatePlottingMatrix, 0)
        ...
        # Vectors that plot the average values of all assessors
        x_values = np.arange(1, replicateDimensions[1] + 1)
        y_values = meanVector
