...
from Math_Tools import PCA, STD
from pca_module import CorrelationLoadings
from Plot_Tools import str_row, num2str, equal_lists, raw_data_grid, axes_create, axes_setup, numerical_data_add_scores, OverviewPlotter, significance_legend, colored_frame
    ...
    if selection[1] == 1:  # model based on sample replicates
        ...
        for assessor in plot_data.activeAssessorsList:
            for replicate in s_data.ReplicateList:
                ass_rep_mat = s_data.GetAssessorDataAs2DARRAY(
        ...
        for assessor in plot_data.activeAssessorsList:
            ...
            for attribute in plot_data.activeAttributesList:
                ...
                for sample in plot_data.activeSamplesList:
                    ...
                    sumReplicates = 0
                    for replicate in s_data.ReplicateList:
                        oneReplicate = s_data.SparseMatrix[(
                            assessor, sample, replicate)]
                        positionSpecificAttribute = s_data.AttributeList.index(
                            attribute)
                        valueSpecificAttribute = float(
                            oneReplicate[positionSpecificAttribute])
                    ...
                    variablePosition = plot_data.activeAssessorsList.index(
                        assessor) * numberOfAttributes + plot_data.activeAttributesList.index(attribute)
                    Tucker1Matrix[objectPosition,
                                  variablePosition] = sampleAverage

    leaveOutAttributes = []
    newActiveAttributesList = []
    if selection[0] == 1:
        ...
        for col in leaveOutColumns:
            ...
            attPosition = col - (multiplyFactor *
                                 len(plot_data.activeAttributesList))
            if plot_data.activeAttributesList[int(attPosition)] not in leaveOutAttributes:
                leaveOutAttributes.append(
                    plot_data.activeAttributesList[int(attPosition)])
        ...
        if selection[1] == 1:  # model based on sample replicates
            ...
            for assessor in plot_data.activeAssessorsList:
                for replicate in s_data.ReplicateList:
                    ass_rep_mat = s_data.GetAssessorDataAs2DARRAY(
                        active_samples=plot_data.activeSamplesList,
                        active_attributes=newActiveAttributesList,
                        active_replicates=[replicate],
                        active_assessor=assessor)
                    # print ass_rep_mat
                    [y_len, x_len] = np.shape(ass_rep_mat)
            ...
            for assessor in plot_data.activeAssessorsList:
                ...
                for attribute in newActiveAttributesList:
                    ...
                    for sample in plot_data.activeSamplesList:
                        ...
                        # Retrieve position in Matrix that is to be filled with
                        # average value
                        objectPosition = plot_data.activeSamplesList.index(
                            sample)
                        variablePosition = plot_data.activeAssessorsList.index(
                            assessor) * len(newActiveAttributesList) + newActiveAttributesList.index(attribute)
...
def Tucker1Plotter(
    s_data, plot_data, num_subplot=[
        1, 1, 1], selection=[
            0, 0], pc_x=0, pc_y=1,abspath=None):
    """
    ...
    if itemID[0] not in collectedActiveItemsList:
        dlg = wx.MessageDialog(
            None, 'The assessor or attribute is not active in CheckBox',
        ...
        if numberOfAttributes < 1:
            dlg = wx.MessageDialog(
                None, 'There must be a minimum of 1 attribute.',
                'Error Message', wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
...
##        output = open(fileNameTucker1, 'w')
        ...
        if not calculated or PCAmodeSelected[0] == 1:
            Tucker1Matrix, newActiveAttributesList, leaveOutAttributes = getTucker1Matrix(
                s_data, plot_data, PCAmodeSelected)

            # print plot_data.newActiveAttributesList
            # print newActiveAttributesList

            if plot_data.newActiveAttributesList is not None and len(
                    plot_data.newActiveAttributesList) > 0:
            ...
            if not calculated and selection[0] == 1:
                ...
                if len(leaveOutAttributes) > 0:
                    ...
                    dlg.ShowModal()
                    dlg.Destroy()
                    if len(activeAttributesList) == 0:
                        return

                activeAttributesList = newActiveAttributesList[:]

                # re-calculation needed
                plot_data.newActiveAttributesList = newActiveAttributesList[:]
        ...
        if subplot:  # is subplot
            ...
            plot_data.ax = axes_create(
                view_legend, plot_data.fig, aspect=plot_data.aspect)
        ax = plot_data.ax
        fig = plot_data.fig
        ...
        if itemID[0] == u'Common Scores':
            ...
            xAx = 'PC' + str(pc_x + 1) + ' (' + str(
                round(explVar[pc_x] * 100, 1)) + '%)'
            yAx = 'PC' + str(pc_y + 1) + ' (' + str(
                round(explVar[pc_y] * 100, 1)) + '%)'
            ...
            if itemID[0] in activeAssessorsList:
                ...
                for coords in range(len(specificAssessorXCoordinates)):
                    if (specificAssessorXCoordinates[coords] == 0) and (
                            specificAssessorXCoordinates[coords] == 0):
                        pass
                    else:
            ...
            # In case of a selected attribute
                ...
                if selection[1] == 0:
                    ...
                    for position in range(numberOfAssessors):
                        newPosition = position * len(
                            activeAttributesList) + specificPositionAttribute
                    ...
                    for coords in specificAttributeSequence:
                        ...
                        specificAssessor = specificAttributeSequence.index(
                            coords)
                    ...
                    for ass in activeAssessorsList:
                        for rep_ind in range(len(s_data.ReplicateList)):
                            ...
                            newPosition = counter * len(
                                activeAttributesList) + specificPositionAttribute
                            specificAttributeSequence.append(
                                [newPosition, ass + '-' + str(rep_ind + 1)])
                            counter += 1
...
def Tucker1AssOverviewPlotter(s_data, plot_data, pca_mode_select,abspath=None):
    # Overview plot #
    ...
    return OverviewPlotter(
        ...
        special_selection=pca_mode_select,abspath=abspath)


def Tucker1AttOverviewPlotter(s_data, plot_data, pca_mode_select,abspath=None):
    # Overview plot #
    itemID_list = []  # takes part in what to be plotted (tree path)
