...
import numpy
import numpy.linalg
from pca_module import PCA_nipals2
...
def MVG(arr, perc):
    ...
    while True:
        ...
        if len(indicesList) == noOfMissingElements:
            break
...
def IA(arr, initVal, noFactors, noIterations):
    ...
    if initVal == 0:
        ...
        for ind in positions:
            # print ind
            newArr[ind[0], ind[1]] = (
                nan_colMeansList[ind[1]] + nan_rowMeansList[ind[0]]) / 2
...
# XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    for iters in range(noIterations):
        ...
        if noFactors == 1:
            ...
            predMatrix = numpy.dot(pc1scores, pc1loadings) + \
                numpy.dot(pc2scores, pc2loadings)
            predArr = numpy.array(predMatrix)
            ...
            predMatrix = numpy.dot(pc1scores,
                                   pc1loadings) + numpy.dot(pc2scores,
                                                            pc2loadings) + numpy.dot(pc3scores,
                                                                                     pc3loadings)
            predArr = numpy.array(predMatrix)
...
def LSA(arr, noFactors, noIterations):
    ...
    for iter in range(noIterations):
        ...
        for sample in range(rows):
            ...
            for attribute in range(cols):
                ...
                for PC in range(noFactors):
                    A_i[attribute, PC] = C[sample, attribute] * \
                        loadings[PC, attribute]
        ...
        for sample in range(rows):
            ...
            t_i = numpy.dot(
                numpy.dot(
                    x_i, A_i), numpy.linalg.inv(
                    numpy.dot(
                        numpy.transpose(A_i), A_i)))
            t[sample, :] = t_i.copy()
...
##    X_p = LSA(X_m, 2, 10)
    ...
    for runs in range(1):
        ...
        T, P, E = pca_module.PCA_nipals2(
            X, standardize=False, E_matrices=False)
        scipy.io.write_array('X_m.txt', X_m, separator='\t', linesep='\n')
